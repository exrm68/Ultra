...
import { 
  ...
  ChevronDown, ChevronUp, Award, Palette, Bell, Zap, Grid, Film
} from 'lucide-react';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ onClose }) => {
  ...
  const handlePublish = async () => {
    if (!title || !thumbnail) {
      alert('টাইটেল এবং থাম্বনেইল দিতে হবে!');
      return;
    }
    ...
    try {
      const movieData: Partial<Movie> = {
        ...
        storyEnabled,
        storyImage: storyEnabled ? (storyImage || thumbnail) : undefined,
        priority,
  ...
  const handleAddStory = async () => {
    ...
    try {
      const storyData: Partial<StoryItem> = {
        ...
        movieId: newStoryMovieId || undefined,
  ...
  const handleAddBanner = async () => {
    ...
    setLoading(true);
    try {
      const bannerData: Partial<BannerItem> = {
        title: newBannerTitle,
        image: newBannerImage,
        movieId: newBannerMovieId || undefined,
  ...
  const toggleTop10 = async (movieId: string, currentStatus: boolean) => {
    try {
      await updateDoc(doc(db, 'movies', movieId), {
        isTop10: !currentStatus,
        top10Position: !currentStatus ? 10 : undefined
      });
                    ...
                    <div className="space-y-4">
                      ...
                      {episodes.length > 0 && (
                        <div className="space-y-2 max-h-60 overflow-y-auto">
                          {episodes.map((ep, idx) => (
                            <div key={ep.id} className="bg-gray-900 rounded-lg p-3 flex items-center justify-between">
                              <div className="flex-1">
                                <div className="text-white font-medium">
                                  S{ep.season} E{ep.number}: {ep.title}
                                </div>
                                <div className="text-gray-400 text-sm">{ep.duration}</div>
                              </div>
                              <button
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex gap-2 mt-2">
                                <input
                                  ...
                                  onBlur={(e) => updateTop10Position(movie.id, parseInt(e.target.value))}
